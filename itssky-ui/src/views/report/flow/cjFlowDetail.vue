<template>
  <div class="app-container">
    <div style="display: flex;justify-content: center;flex-flow: column;flex-direction: column;flex-wrap: nowrap;align-content: center;align-items: center;padding-bottom: .5vh">
      ...
      <h3 style="font-weight: bolder;margin: 1vh 0">CJ出口(MTC)交通流量统计表</h3>
    </div>
    ...
    <el-table v-loading="loading" :data="dataList" border ref="myTable" >
      ...
      <el-table-column label="公务" align="center" prop="gw"/>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      crjFlow(this.queryParams).then(response => {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出CJ出口(MTC)交通流量统计表?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return exportCrjFlow(queryParams);
      }).then(response => {
  ...
  `;
      ...
      this.dataList.forEach(row => {
        tableHtml += `
          <tr>
            ...
            <td>${row.gw !== undefined && row.gw !== null ? row.gw : ''}</td>
            <td>${row.jc !== undefined && row.jc !== null ? row.jc : ''}</td>
            <td>${row.yh !== undefined && row.yh !== null ? row.yh : ''}</td>
            <td>${row.mf !== undefined && row.mf !== null ? row.mf : ''}</td>
            <td>${row.cd !== undefined && row.cd !== null ? row.cd : ''}</td>
            <td>${row.kcbl !== undefined && row.kcbl !== null ? row.kcbl : ''}</td>
            <td>${row.hcbl !== undefined && row.hcbl !== null ? row.hcbl : ''}</td>
            <td>${row.zcbl !== undefined && row.zcbl !== null ? row.zcbl : ''}</td>
      ...
      <!DOCTYPE html>
        ...
        <body>
            ...
            <div class="print-title">CJ出口(MTC)交通流量统计表</div>
