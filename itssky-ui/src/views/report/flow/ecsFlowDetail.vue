<template>
  <div class="app-container">
    <div style="display: flex;justify-content: center;flex-flow: column;flex-direction: column;flex-wrap: nowrap;align-content: center;align-items: center;padding-bottom: .5vh">
      ...
      <h3 style="font-weight: bolder;margin: 1vh 0">ECS电子支付(MTC+ETC)出口流量统计表</h3>
    </div>
...
import {erjsFlow, exportEcsFlow} from "@/api/report/exitFlow"
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出ECS电子支付(MTC+ETC)出口流量统计表?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return exportEcsFlow(queryParams);
      }).then(response => {
