<template>
  <div class="app-container">
    <div style="display: flex;justify-content: center;flex-flow: column;flex-direction: column;flex-wrap: nowrap;align-content: center;align-items: center;padding-bottom: .5vh">
      ...
      <h3 style="font-weight: bolder;margin: 1vh 0">ERS电子支付(MTC+ETC)入口流量统计表</h3>
    </div>
...
import {erjsFlow, exportErsFlow} from "@/api/report/exitFlow"
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出ERS电子支付(MTC+ETC)入口流量统计表?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return exportErsFlow(queryParams);
      }).then(response => {
  ...
  `;
      ...
      <!DOCTYPE html>
        ...
        <body>
            ...
            <div class="print-title">ERS电子支付(MTC+ETC)入口流量统计表</div>
