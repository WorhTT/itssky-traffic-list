<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
      <el-form-item label="路公司" prop="corpNo">
        <el-select v-model="queryParams.corpNo"
                   placeholder="请选择路公司"
                   clearable
                   style="width: 240px"
                   filterable
        >
          <el-option v-for="dict in corpOptions"
                     :key="dict.corpNo"
                     :label="dict.label"
                     :value="dict.corpNo">
            <div style="display: flex; justify-content: space-between">
              <span>{{ dict.label }}</span>
              <span>{{ dict.stationHex }}</span>
            </div>
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="统计日期:" prop="beginTime">
        <el-date-picker
          v-model="queryParams.beginTime"
          placeholder="请选择日期"
          type="date"
          value-format="yyyy-MM-dd"
          :picker-options="pickOptions"
        >
        </el-date-picker>
      </el-form-item>
      <el-form-item label="结束日期:" prop="statisticsTime">
        <el-date-picker
          v-model="queryParams.endTime"
          placeholder="请选择日期"
          type="date"
          value-format="yyyy-MM-dd"
          :picker-options="pickOptions"
        >
        </el-date-picker>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" @click="openChildPage">查看详细报表</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>

<script>

import {centerOptions, listStationSelect} from "@/api/system/station";
import {getCurrentTime, getMidnightTime} from "@/utils/dateUtils";

export default {
  name: "UnUseEtc",
  data() {
    return {
      props: {multiple: true},
      // 遮罩层
      loading: false,
      // 选中数组
      ids: [],
      // 非单个禁用
      single: true,
      // 非多个禁用
      multiple: true,
      // 显示搜索条件
      showSearch: true,
      // 总条数
      total: 0,
      // 公告表格数据
      dataList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
      // 查询参数
      queryParams: {
        beginTime: null,
        endTime: null,
        corpNo: null,
      },
      // 表单参数
      form: {},
      // 表单校验
      rules: {
      },
      stationOptions: [],
      shiftOptions: [],
      pickerType: 'date',
      pickOptions: {
        disabledDate(time) {
          return time.getTime() > Date.now();
        },
      },
      corpOptions: [],
    };
  },
  created() {
    this.queryParams.beginTime = getMidnightTime();
    this.queryParams.endTime = getCurrentTime();
    this.getCorpOptions();
  },
  methods: {
    getCorpOptions() {
      centerOptions().then(res => {
        this.corpOptions = res.data;
      })
    },
    openChildPage() {
      const route = {
        path: '/unUseEtcDetail',
        query: this.queryParams
      }
      const resolve = this.$router.resolve(route);
      window.open(resolve.href, '_blank')
    }
  }
};
</script>
