...
import {s1StationShift, exportS2StationShift} from "@/api/report/card";
import {getLoginUser} from "@/api/login";

export default {
  name: "S2StationDayDetail",
  data() {
    return {
      props: {multiple: true},
      // 遮罩层
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出S2收费站通行卡发放日统计表?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return exportS2StationShift(queryParams);
      }).then(response => {
        ...
        <table class="el-table">
          <thead>
            <tr>
              ...
              <th style="min-width: 60px;">恢复卡</th>
              <th style="min-width: 50px;">纸券</th>
              <th style="min-width: 60px;">应发卡</th>
              <th style="min-width: 60px;">实发卡</th>
